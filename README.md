# Hi, I'm Jorge 👋

🎓 IT & Cybersecurity student at Polk State College pursuing an Associate in Science
🔐 CompTIA A+ | CompTIA Network+ | Google Cybersecurity Certificate
🌎 Based in Florida
🚀 Actively building hands-on skills through home lab projects and self-directed learning
💼 Seeking entry-level roles in Cybersecurity, Network Engineering, or IT Infrastructure


---

## 🧠 About Me

I am an IT professional with a strong foundation in networking, cybersecurity, and system administration built through hands-on home lab projects and self-directed learning. My journey in information technology has led me to develop a deep focus on network security, threat detection, and Windows domain administration. I am passionate about understanding how attacks work so I can better defend against them.

---

## 🛠️ Technical Skills

| Category | Skills |
|----------|--------|
| **Networking** | DNS, DHCP, Firewall Configuration, NAT, VPN (WireGuard), Network Troubleshooting |
| **Security** | pfSense, Snort IPS, Splunk SIEM, Intrusion Detection, Log Analysis, Adversary Emulation |
| **Operating Systems** | Windows 10/11, Windows Server, Linux (Ubuntu Server) |
| **Virtualization** | Oracle VirtualBox, pfSense, Multi-VM Lab Design |
| **Active Directory** | Domain Services, User & Group Management, Group Policy |
| **Tools** | Splunk, Sysmon, Atomic Red Team, Nmap, Kali Linux, Apache |

---

## 🏗️ Projects

### 🔒 [Virtual Firewall & Network Security Home Lab](https://github.com/jorger31/pfSense-Network-Lab)
> Built a fully functional network security lab using pfSense, WireGuard VPN, and Snort IPS entirely on virtual machines

- Deployed pfSense as a virtual firewall with stateful packet inspection and custom firewall rule policies
- Configured NAT and port forwarding to simulate hosting a web server behind a firewall
- Implemented WireGuard VPN server with public key infrastructure for encrypted tunneling
- Integrated Snort IPS with Emerging Threats and GPLv2 rulesets for real-time intrusion detection

**Tools:** pfSense · VirtualBox · Ubuntu Server · WireGuard · Snort · Apache · Nmap

---

### 🏢 [Active Directory Home Lab — Threat Detection & Adversary Emulation](https://github.com/jorger31/Active-Directory-VM)
> Built a Windows domain environment with a dedicated attacker machine, Splunk SIEM, and adversary emulation using Atomic Red Team

- Deployed Active Directory Domain Services and managed users, groups, and Group Policy Objects
- Configured Splunk SIEM with Universal Forwarders on all endpoints for centralized log collection
- Installed Sysmon for enhanced endpoint telemetry capturing process creation, network connections, and registry modifications
- Conducted adversary emulation using Atomic Red Team (MITRE ATT&CK) and Kali Linux to simulate real-world attacks
- Built Splunk detection queries to identify indicators of compromise across the domain

**Tools:** Windows Server · Windows 10 · Kali Linux · Splunk · Sysmon · Atomic Red Team · VirtualBox

---

### 🔒 [Self-Hosted WireGuard VPN](https://github.com/jorger31/Self-Hosted-WireGuard-VPN)
> Built a personal VPN server from scratch on a cloud VPS using WireGuard, with multi-device client support

- Deployed and hardened a Ubuntu Linux VPS (Vultr) with SSH key-based authentication and disabled root/password login
- Configured WireGuard with IP forwarding, NAT/iptables masquerading, and UFW firewall rules
- Generated per-device keypairs and configs for multiple simultaneous clients (desktop, mobile) with QR-code provisioning
- Documented the full setup, troubleshooting process, and security practices in a public repo

**Tools:** WireGuard · Ubuntu Server · Vultr · UFW · iptables


## 📜 Certifications

<div>
<a href="https://www.credly.com/badges/cfe14753-f564-44e1-822f-2e728549dc71/public_url" target="_blank">
  <img src="https://img.shields.io/badge/-CompTIA%20A+-EA2B2C?style=for-the-badge&logo=comptia&logoColor=white" alt="CompTIA A+" />
</a>
<a href="https://www.credly.com/badges/2c997e64-83d6-42f1-9efa-bb1cd4d1a550/public_url" target="_blank">
  <img src="https://img.shields.io/badge/-CompTIA%20Network+-EA2B2C?style=for-the-badge&logo=comptia&logoColor=white" alt="CompTIA Network+" />
</a>
<a href="https://coursera.org/verify/professional-cert/6AH8WN5G5ROX" target="_blank">
  <img src="https://img.shields.io/badge/Google%20Cybersecurity%20Professional%20Certificate-4285F4?style=for-the-badge&logo=google&logoColor=white" alt="Google Cybersecurity Professional Certificate Badge" />
</a>
</div>

---

## 🧰 Tools & Technologies

### Security & Networking
<div>
<a href="https://www.pfsense.org/" target="_blank">
  <img src="https://img.shields.io/badge/-pfSense-212121?style=for-the-badge&logo=pfsense&logoColor=white" alt="pfSense Badge" />
</a>
<a href="https://www.snort.org/" target="_blank">
  <img src="https://img.shields.io/badge/-Snort-CC0000?style=for-the-badge&logo=snort&logoColor=white" alt="Snort Badge" />
</a>
<a href="https://www.wireguard.com/" target="_blank">
  <img src="https://img.shields.io/badge/-WireGuard-88171A?style=for-the-badge&logo=wireguard&logoColor=white" alt="WireGuard Badge" />
</a>
<a href="https://nmap.org/" target="_blank">
  <img src="https://img.shields.io/badge/-Nmap-214478?style=for-the-badge&logo=nmap&logoColor=white" alt="Nmap Badge" />
</a>
<a href="https://www.kali.org/" target="_blank">
  <img src="https://img.shields.io/badge/-Kali%20Linux-557C94?style=for-the-badge&logo=kalilinux&logoColor=white" alt="Kali Linux Badge" />
</a>
</div>

### Cloud & Infrastructure
<div>
<a href="https://www.vultr.com/" target="_blank">
  <img src="https://img.shields.io/badge/-Vultr-007BFC?style=for-the-badge&logo=vultr&logoColor=white" alt="Vultr Badge" />
</a>
<a href="https://ubuntu.com/server" target="_blank">
  <img src="https://img.shields.io/badge/-UFW-E95420?style=for-the-badge&logo=linux&logoColor=white" alt="UFW Firewall Badge" />
</a>
<a href="https://www.netfilter.org/projects/iptables/" target="_blank">
  <img src="https://img.shields.io/badge/-iptables-E95420?style=for-the-badge&logo=linux&logoColor=white" alt="iptables Badge" />
</a>
</div>

### SIEM & Monitoring
<div>
<a href="https://www.splunk.com/" target="_blank">
  <img src="https://img.shields.io/badge/-Splunk-000000?style=for-the-badge&logo=splunk&logoColor=white" alt="Splunk Badge" />
</a>
<a href="https://docs.microsoft.com/en-us/sysinternals/downloads/sysmon" target="_blank">
  <img src="https://img.shields.io/badge/-Sysmon-0078D4?style=for-the-badge&logo=microsoft&logoColor=white" alt="Sysmon Badge" />
</a>
</div>

### Operating Systems & Virtualization
<div>
<a href="https://www.virtualbox.org/" target="_blank">
  <img src="https://img.shields.io/badge/-Oracle%20VirtualBox-183A61?style=for-the-badge&logo=virtualbox&logoColor=white" alt="Oracle VirtualBox Badge" />
</a>
<a href="https://ubuntu.com/" target="_blank">
  <img src="https://img.shields.io/badge/-Ubuntu-E95420?style=for-the-badge&logo=ubuntu&logoColor=white" alt="Ubuntu Badge" />
</a>
<a href="https://www.microsoft.com/en-us/windows-server" target="_blank">
  <img src="https://img.shields.io/badge/-Windows%20Server-0078D4?style=for-the-badge&logo=windows&logoColor=white" alt="Windows Server Badge" />
</a>
</div>


---

*Always learning. Always building.* 🔐
